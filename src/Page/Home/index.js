...
export default function Home() {
    ...
    useEffect(() => {
        const test = async () => {
            let res = await restfulApi.takeDataProducts();
    ...
    return (
        <>
            <div className={cx('home')}>
                        ...
                        <div className={cx('col-lg-9', 'clg9')}>
                            <div className={cx('product')}>
                                <div className={cx('row', 'pr')}>
                                    {/*  */}
                                    <h2>SẲN PHẨM TRONG THỜI GIAN KHUYẾN MÃI</h2>
                                    {data &&
                                        data.length > 0 &&
                                        data.map((item, index) => {
                                            return (
                ...
                {/* toast cart */}
                <ToastCart boolean={toastShow} update={handleCart} handleClear={handleClear} id={dataToast} />
                {/*  */}
            </div>
        </>
    );
